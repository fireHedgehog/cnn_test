    ...
    with tf.Session() as session:
        ...
        for epoch in range(TRAINING_EPOCHS):
            for b in range(TOTAL_BATCHES):
                ...
                if b % 10000 == 0:
                    print(epoch, b, _, c)
                    # with open('Accuracy_04_08_20_detail.txt', 'a+') as f:
                    #     text = ''
                    #     text += (str(b * (epoch + 1)) + '\t' + str(c) + '\n')
                    #     f.write(text)
...
result = algorithms.eaSimple(popl, toolbox, cxpb=0.3, mutpb=0.8, ngen=num_generations, verbose=True)
print(result)
